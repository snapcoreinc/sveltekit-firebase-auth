...
import { dedupe } from './deDupeAuthEvents';
import type { Session } from './types';
import { clearSignInDataFromLocalStorage } from './localStorage';
	...
	auth.subscribe(($auth) => {
		if ($auth) {
			onIdTokenChanged($auth, async (user) => {
				console.log('Async auth event - calling /auth/session');
				// set or clear the session using the user state / idToken
				let req;

				if (user) {
					req = fetch('/auth/session', {
						method: 'post',
						body: await getIdToken(user)
					});
					clearSignInDataFromLocalStorage();
				} else {
					req = fetch('/auth/session', {
						method: 'delete'
					});
